...
public abstract class OnlineLearningModel implements Serializable {
	...
	public static class InputMap implements MapFunction<String, String> {
	...
	protected abstract CoFlatMapFunction<CountLabelExample, DenseVector, DenseVector> train();
	...
	public void modeling(StreamExecutionEnvironment env) {
		...
		producerPropersteis.setProperty("buffer.memory","200000000");
		...
		DataStream<CountLabelExample> trainData = iterData.map(new MapFunction<CountLabelExample, CountLabelExample>() {
			private static final long serialVersionUID = 1L;
		...
		DataStream<CountLabelExample> stepStream = iterData
